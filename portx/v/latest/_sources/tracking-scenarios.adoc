...
This section identifies scenarios in which PortX is useful to track B2B exchanges. A basic knowledge of PortX is assumed; see  link:/portx/anypoint-partner-manager[PortX] for more information.


== Replaying an Incomplete Transaction

On the link:/portx/monitor-b2b-transactions[B2B Transactions Page], the Transactions View includes a table in which each row corresponds to a transaction.  The first column shows transaction completion status, as follows:
...
After you identify and fix the cause of the error that prevented the transaction from completing, you can use PortX to re-run the transaction.
...
For more information, see "Transactions View" in link:/portx/transaction-monitoring[Transaction Monitoring].
...
The *Dashboard* at the top of the link:/portx/transaction-monitoring[Transaction Monitoring] provides a summary of what has happened in a given timeframe (the default is the current day) including how many errors have been detected during that timeframe.
...
. On *View Dropdown List* of the link:/portx/monitor-b2b-transactions[B2B Transactions Page], choose *Transmissions*.
+
...
. On *View Dropdown List* of the link:/portx/monitor-b2b-transactions[B2B Transactions Page], choose *Documents*.
+
