...
. From your web browser, log into PortX. Go to the Home Organization page and copy your secret API key.
. From the imported Mule project in Anypoint Studio, click the file `b2b-tracking-transmission.xml` in the `app` folder, and select the *Global Elements* view tab. Open the Partner Manager Connector configuration and set the API Key you generated from PortX.
...
////
as described in the link:/portx/as2-and-edi-x12-purchase-order-walkthrough[AS2 and X12 Purchase Order Walkthrough].
////
...
////
Consult the link:/portx/as2-and-edi-x12-purchase-order-walkthrough[AS2 and X12 Purchase Order Walkthrough] for information on how to add the parameters.
////
+
...
+
. Access the PortX and go to the B2B transmissions view to confirm that the exchange was tracked:
+
