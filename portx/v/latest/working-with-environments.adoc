...
PortX (APM) provides the ability to export Partners for backup purposes. These Partners can be imported back into APM later. Please consult the <<Importing Partners Into A Specific Environment>> section for more information.
...
PortX (APM) provides the ability to import <<Partners>> that were previously backed up using the export functionality. Please consult the <<Exporting partners from an environment>> section for more information.
...
PortX (APM) provides the ability to "promote", i.e. "copy", <<Partners>> from one environment to another.
